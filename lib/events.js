...
module.exports = async function (config, eventsLogger) {
    ...
    return {
        createEventsHandler: async function (event) {
            const now = new Date()
            const run = async () => {
                assertConfigValid(config)
                console.log("Topic: " + config.topic)
                await producer.connect().then(console.log("producer is connected")).catch(console.error)
                await producer.send({
                    topic: config.topic,
                    messages: [
